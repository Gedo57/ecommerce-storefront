export default function VendorInfoGrid({ items, columns = 3 }) {
  const gridClass = columns === 4 ? 'xl:grid-cols-4' : columns === 2 ? 'md:grid-cols-2' : 'md:grid-cols-2 xl:grid-cols-3';
  return (
    <div className={`grid gap-4 ${gridClass}`}>
      {items.map((item) => (
        <div key={item.label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
